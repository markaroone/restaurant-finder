...
| **Ambiguous location UX**                            | When Foursquare can't geocode a vague location, the backend returns a structured `AmbiguousLocationError`. The frontend shows a clean "Try adding a city or country" prompt — no auto-suggestion chip, because empirical testing showed Foursquare's geocoder handles real ambiguous locations (Springfield, Richmond) on its own, and GeoIP-based suggestions only helped fake/nonexistent locations where they produced misleading results ([ADR-021](./_docs/decisions.md)) |
...
| **Exponential backoff with full jitter**             | If Gemini fails, we retry with randomized, increasing delays so multiple requests don't all retry at the same time and overwhelm the API. Two attempts fit within the 20s Express timeout budget                                                                                                                                                                                                                                                                               |
